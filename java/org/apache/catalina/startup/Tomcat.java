...
public class Tomcat {
    ...
    public Server getServer() {
        ...
        server = new StandardServer(); // tomcat最外层的server容器
        ...
        server.addService( service ); // 将service容器添加到server容器中
        return server;
    }
    ...
    public Context addWebapp(Host host, String contextPath, String docBase) {
        LifecycleListener listener = null; // 监听器
        try {
            Class<?> clazz = Class.forName(getHost().getConfigClass()); // 通过反射得到java对象
    ...
    public Context addWebapp(Host host, String contextPath, String docBase,
        ...
        Context ctx = createContext(host, contextPath); // 创建context容器
        ctx.setPath(contextPath); // 设置属性
    ...
    private Context createContext(Host host, String url) {
        String contextClass = StandardContext.class.getName();// StandardContext代表context容器
        if (host == null) {
            host = this.getHost();
        }
        ...
        try {
            return (Context) Class.forName(contextClass).getConstructor()
                    .newInstance(); // 通过反射加载了一个StandardContext对象
